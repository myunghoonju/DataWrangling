...
(function (window, $, undefined) {
    ...
    $.gummGoogleMap.prototype = {
        ...
        __construct: function(options, callback) {
            ...
            if ($(document).data('gummGoogleMapScriptInit') !== true) {
                ...
                script.src = "//maps.googleapis.com/maps/api/js?sensor=" + sensor + "&callback=gummGoogleMapOnLoadCallback";
    ...
    $.gummAutocomplete.prototype = {
        ...
        bindListeners: function() {
            ...
            var hide_delay = 100;
            if( $("html").hasClass("lt9") ) { hide_delay = 500;};
    ...
    // ======================= //
    // GUMM MOBILEAUTOCOMPLETE //
    // ======================= //
    
    $.gummMobileAutocomplete = function gummMobileAutocomplete(options, callback, element) {
        this.element = $(element);
        this.__construct(options, callback);
    }
    
    $.gummMobileAutocomplete.settings = {
        minLength: 1,
        timeout: 500,
        beforeFind: function(){}
    }
    
    $.gummMobileAutocomplete.prototype = {
        state: 'default',
        length: 0,
        val: '',
        lastVal: '',
        ...
        popover: null,
        __construct: function(options, callback) {
            this.options = $.extend(true, {}, $.gummMobileAutocomplete.settings, options);
        ...
        getResultsTitle: function() {
            return $('.mobile-searchresults').find('h4');
        },
        setResultsTitle: function(title) {
            var $resultsElement = $('.mobile-searchresults');
            
            if ($resultsElement.find('h4').length === 0){
                $resultsElement.prepend('<h4>');
            }
            
            $('.mobile-searchresults h4').html('<a href="' + this.ajaxUrl + '?s=' + this.val + '">' + this.element.data('view-all-title') + '</a>');
        },
        getResultsContent: function() {
            return $('.mobile-searchresults').find('.results');
        },
        setResultsContent: function(content) {
            this.getResultsContent().html(content);
        },
        find: function() {
            var _self = this;
            ...
            if (this.length >= this.options.minLength) {
                ...
                this.setResultsTitle('<a href="' + this.ajaxUrl + '?s=' + this.val + '">' + this.element.data('view-all-title') + '</a>');
        ...
        getResults: function() {
            ...
            $.ajax({
                ...
                success: function(data, textStatus, XMLHttpRequest) {
                    $('.mobile-searchresults .results').html(data);
                }
            })
    ...
    $.fn.gummMobileAutocomplete = function gummMobileAutocompleteFn(options, callback) {
        this.each(function () {
            var instance = $.data(this, 'gummMobileAutocomplete');
            if (instance) {
                // update options of current instance
                // instance.update(options);
            } else {
                $.data(this, 'gummMobileAutocomplete', new $.gummMobileAutocomplete(options, callback, this));
            }
    ...
    $.fn.gummDateTimePicker = function gummDateTimePicker(){
        this.datetimepicker({
	    dateFormat: 'yy/mm/dd',
	    timeFormat: 'h:mm TT',
	    changeMonth: true,
	    changeYear: true,
	    beforeShow: function realignWithTextbox(input, instance){
		var $popupDiv = jQuery(instance.dpDiv);
		var $input = jQuery(input);

		var inputTopLeftCorner = $input.offset();
		var inputBottomRightCorner = {
		    top : inputTopLeftCorner.top + $input.outerHeight(),
		    left : inputTopLeftCorner.left + $input.outerWidth()
		};

		// width of the popup is 300px plus 20px margin (because of `#ui-datepicker-div dl` in style-admin.css)
		var popupWidth = 320;

		window.setTimeout(function(){
		    // A bit hacky, but we need to set the position after it is set internally in the jquery ui datepicker
		    $popupDiv
			.css('top', inputBottomRightCorner.top)
			.css('left', inputBottomRightCorner.left - popupWidth);
		}, 1);
	    },
            onClose: function checkDateFormat(dateText, instance) {
                var $input = $(this);

                var $errorDiv = $input.parent().find('.error');
                if ($errorDiv.length > 0){
                    $errorDiv.remove();
                }

                // Let's just use javascript's built in date parser to judge whether we have a valid date
                ...
                if (dateText === ''){
                    ...
                    $input.val('');
                    $input.parent().append('<div class="error"> Please enter a valid date in "yyyy/mm/dd hh:mm" format </div>');
                    return;
                }
                ...
                var formattedDate = formattedYear + '/'
                        + formattedMonth + '/'
                        + formattedDay + ' '
                        + formattedHours + ':'
                        + formattedMinutes;
